...
class DWTWatermark(WatermarkAlgorithm):
    """DWT 水印算法"""

    def __init__(self, alpha: float = 0.1, wavelet='haar', level=1):
        """
        初始化 DWT 水印算法。

        参数:
            alpha (float): 水印强度。
            wavelet (str): 小波类型,  'haar' (默认), 'db1', 'sym2' 等.
            level (int): 分解级别。
        """
        self.alpha = alpha
    ...
    def embed_watermark(self, host_image: np.ndarray, watermark: np.ndarray) -> np.ndarray:
        ...
        if host_image.shape != watermark.shape:
            watermark = cv2.resize(watermark, (host_image.shape[1], host_image.shape[0]))

        # DWT 分解
        coeffs = pywt.wavedec2(host_image, wavelet=self.wavelet, level=self.level)
        LL, (LH, HL, HH) = coeffs[0], coeffs[1] # 仅在第一层嵌入
        
        # 嵌入水印 (修改 LL 子带)
        LL = LL.astype(np.float32)
        watermark_resized = cv2.resize(watermark, (LL.shape[1], LL.shape[0])).astype(np.float32)
        LL += self.alpha * watermark_resized

        # 重构
        coeffs_modified = [LL, (LH, HL, HH)]  # 仅修改了第一层
        for i in range(2, len(coeffs)):  # 保留其他层
            coeffs_modified.append(coeffs[i])
        
        watermarked_image = pywt.waverec2(coeffs_modified, wavelet=self.wavelet)
    ...
    def extract_watermark(self, watermarked_image: np.ndarray, watermark_shape: Tuple[int, int]) -> np.ndarray:
        ...
        # DWT 分解
        coeffs = pywt.wavedec2(watermarked_image, wavelet=self.wavelet, level=self.level)
        LL, (LH, HL, HH) = coeffs[0], coeffs[1]  # 获取第一层
        
        # 提取水印
        LL = LL.astype(np.float32)
        watermark_extracted = LL / self.alpha # 提取水印
        watermark_extracted = watermark_extracted - np.mean(watermark_extracted) # 去除直流分量的影响, 使得提取的水印均值为0

        # 调整大小并缩放
        watermark_extracted = cv2.resize(watermark_extracted, (watermark_shape[1], watermark_shape[0]))
        watermark_extracted = np.uint8(np.clip(watermark_extracted, 0, 255)) # 转换为 uint8 格式
