...
def sm3_hash(message: bytes) -> bytearray:
  ...
  # 消息填充
  # 计算填充后的总长度。先加1 (0x80)，再加8 (比特长度)，然后向上取整到64字节的倍数
  # (msg_len + 1 + 8) 计算了消息本身、1比特的'1'和64比特的长度信息所需的最小字节数
  # + 63 然后 // 64 * 64 确保结果是64的倍数
  padded_len = ((msg_len + 1 + 8 + 63) // 64) * 64
  padded = bytearray(padded_len)

  # 复制原始消息
  padded[:msg_len] = message
  # 添加 '1' 比特 (0x80 = 10000000b)
  padded[msg_len] = 0x80
  # 添加消息的比特长度 (64比特)
  # 注意：长度信息是大端序
  for i in range(8):
